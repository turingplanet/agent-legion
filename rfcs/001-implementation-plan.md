...
## M1.5 — Bot hardening (1 session) — *added 2026-08-03 after the force-push incident*

The July cron force-pushed over a hand-resolved sync branch, shipped a stale
`poetry.lock` (red gate on every dep-changing sync), and failed silently on
...
## M4 — Registrar v1 — ✅ SHIPPED 2026-08-05 (lives in fleet-services, as revised)
...
- [x] `deployments.yaml` in agent-registry (slug/repo/host; limits + dns-only deferred).
- [x] Deploy workflow LIVE (2026-08-03): `deploy-fleet.yml` + `scripts/deploy-fleet.py` — triggers on merge touching deployments.yaml; reconciles agent-fleet via GraphQL (serviceCreate + PORT/HOST vars for missing entries, serviceDelete for removed = kill switch); regenerates the fleet-router Caddyfile and pushes it (router redeploys itself). Member CD is Railway's own GitHub integration (merge → redeploy), so no railway-up/gate-status step needed in v1. Verified green end to end. Gotchas fixed en route: Railway API 403s without a User-Agent header; secret had a stale value (added a sha256 fingerprint debug step).
